...
//////////////
	...
	for(j=0;st[j]!=44;j++){//(,)아스키 59
		if(st[j]<47 || st[j]>58) continue; // (0)아스키 48 , (9)아스키 57
		//printf("%c %d\n",index[i],i);
		temp = st[j]-48; //숫자로 바꾸기
		st[j]=0;//초기화
		for(k=1; k<=cnt;k++){
			temp*=10;
		}//십의자리 곱해주기
